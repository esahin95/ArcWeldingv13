...
namespace Foam
{
    defineTypeNameAndDebug(DTRMParticleCloud, 0);
}


// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::DTRMParticleCloud::DTRMParticleCloud
(
    const fvMesh& mesh,
...
:
    Cloud<DTRMParticle>(mesh, cloudName, false),
    mesh_(mesh),
    particleProperties_
    (
        IOobject
...
void Foam::DTRMParticleCloud::move(const dimensionedVector& g)
{
    ...
    DTRMParticle::trackingData
        td(*this, rhoInterp, UInterp, nuInterp, g.value());

    Cloud<DTRMParticle>::move(*this, td);
}
